...
class Login extends CI_Model 
{
	...
	{
		$query = "DELETE FROM users WHERE user.id = {$user_id}";
		$this->db->query($query); //this is running the query into the database
	}
	...
	// }
	public function add_comment($comment, $user_id, $id)
	{
		$query = "INSERT INTO comments (content, created_at, updated_at, user_id, message_id) VALUES (?, NOW(), NOW(), ?, ?)";
		$values = array($comment['comment'], $id, $user_id);
		return $this->db->query($query, $values);
	}
